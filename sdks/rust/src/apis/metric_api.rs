...
/// struct for typed errors of method [`o_bpv3_1_0_get_metrics_top_consumers`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum OBpv310GetMetricsTopConsumersError {
    Status500(),
    UnknownValue(serde_json::Value),
}
...
pub async fn o_bpv3_1_0_get_metrics_top_consumers(configuration: &configuration::Configuration, ) -> Result<models::Obpv310GetMetricsTopConsumers200Response, Error<OBpv310GetMetricsTopConsumersError>> {

    let uri_str = format!("{}/obp/v3.1.0/management/metrics/top-consumers", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Obpv310GetMetricsTopConsumers200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Obpv310GetMetricsTopConsumers200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<OBpv310GetMetricsTopConsumersError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
