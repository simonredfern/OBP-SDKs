...
| [**oBPv400CreateUserWithRoles**](EntitlementApi.md#obpv400createuserwithrolesoperation) | **POST** /obp/v4.0.0/user-entitlements | Create (DAuth) User with Roles |
...
| [**oBPv600AddUserToGroup**](EntitlementApi.md#obpv600addusertogroupoperation) | **POST** /obp/v6.0.0/users/{userid}/group-entitlements | Grant User Membership to Group Entitlements |
...
async function example() {
  ...
  const api = new EntitlementApi(config);
...
import type { OBPv310GetAllEntitlementsRequest } from 'obp-typescript';
...
async function example() {
  ...
  try {
    const data = await api.oBPv310GetAllEntitlements();
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
import type { OBPv600GetRolesWithEntitlementCountsAtAllBanksRequest } from 'obp-typescript';
...
async function example() {
  ...
  try {
    const data = await api.oBPv600GetRolesWithEntitlementCountsAtAllBanks();
    console.log(data);
  } catch (error) {
    console.error(error);
  }
