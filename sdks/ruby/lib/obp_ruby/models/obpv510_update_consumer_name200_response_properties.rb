...
module OpenBankProject
  class OBPv510UpdateConsumerName200ResponseProperties < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenBankProject::OBPv510UpdateConsumerName200ResponseProperties` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenBankProject::OBPv510UpdateConsumerName200ResponseProperties`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [app_type, description, certificate_pem, created_by_user, enabled, consumer_key, certificate_info, redirect_url, logo_url, company, developer_email, consumer_id, app_name, created].hash
    end
